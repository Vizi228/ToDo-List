...
import { changeCheckTasksItem, changeTasksItemName, deleteTasksItem } from '../../redux/actions/tasksActions';

const Task = ({name, colors, colorId, checked, id, tasks, currentListId}) => {
  const dispatch = useDispatch();
  const [changeIsActive, setChangeIsActive] = useState(true);
  const [changeValue, setChangeValue] = useState('');
  const getColor = () => {
      return colors.filter(color => color.id === colorId).map(item => item.hex).join()
  }
  const getChecked = () => {
    return checked ? false : true
  }
  const plusChecked = () => {
    dispatch(changeCheckTasksItem({id, UID: currentListId, name, checked: getChecked(), colorId}, tasks))
  }
  const changeInput = e => {
    setChangeValue(e.target.value)
  }
  const setChangedTaskName = () => {
    if(changeValue) {
      dispatch(changeTasksItemName({id, UID: currentListId, name: changeValue, checked, colorId}, tasks));
      setChangeValue('');
  ...
  const removeTaskItem = () => {
    dispatch(deleteTasksItem(id))
  }
  return (
    <div className="tasks__items-row"   >
      <div className="checkbox" onClick={() => plusChecked()}>
        <input
          type="checkbox"
          checked={checked}
          onChange={plusChecked}
        />
      ...
      <p style={{ color: getColor(colorId)}}>{name}</p>
      <div className="tasks__items-row-actions">
        {changeIsActive ? 
          <div onClick={() => setChangeIsActive(!changeIsActive)} >
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
          ...
          <>
            <input type="text" value={changeValue} onChange={changeInput}/>
            <button onClick={() => setChangedTaskName()}>Изменить</button>
          </>
        }
        
        <div onClick={() => removeTaskItem()}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 11 11"
